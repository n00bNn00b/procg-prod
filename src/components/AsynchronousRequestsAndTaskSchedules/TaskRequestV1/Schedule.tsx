...
interface IScheduleProps {
  schedule: IScheduleTypes | undefined;
  setSchedule: Dispatch<SetStateAction<IScheduleTypes | undefined>>;
  scheduleType: string;
...
const Schedule: FC<IScheduleProps> = ({
  ...
  const onSubmit = (data: z.infer<typeof FormSchema>) => {
    ...
    setIsOpenModal(""); // Close modal on submit
  };

  return (
    <CustomModal4 w="w-[700px]" h="h-[300px]">
      <div>
        {action === "Schedule" && (
          <div className="p-2 bg-slate-300 rounded-t mx-auto text-center font-bold flex justify-between">
            <h2>{action}</h2>
            <X onClick={() => setIsOpenModal("")} className="cursor-pointer" />
          </div>
        )}
        <Form {...form}>
          <form onSubmit={form.handleSubmit(onSubmit)} className="p-4">
            ...
            <div className="fixed bottom-4 right-4 flex gap-2">
              ...
              <Button variant="secondary" onClick={() => setIsOpenModal("")}>
                Cancel
              </Button>
            </div>
          </form>
