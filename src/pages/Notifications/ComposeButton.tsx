...
return (
    <Dialog>
      ...
      <DialogContent>
        ...
        <button onClick={()=> setShowUsers(!showUsers)} className="bg-black text-white w-40 h-8 rounded-sm font-semibold">Select Recipients</button>
            {recivers? <div className="rounded-sm w-[calc(100%-11rem)] max-h-20 scrollbar-thin overflow-auto flex flex-wrap gap-1">
                ...
                {/* {recipients.map(rec => (
                    <div className="flex gap-1 bg-light-200 h-8 px-3 items-center rounded-full">
                        <p>{rec}</p>
                        <div onClick={()=> removeReciver(rec)} className="flex h-[65%] items-end cursor-pointer">
                            <Delete size={18} color="#F23030"/>
                        </div>
                    </div>
                ))} */}
            </div>: null}
        </div>
        {showUsers? <div className="w-40 fixed z-10 bg-light-100 border rounded-b-sm top-[90px] max-h-[210px] overflow-auto scrollbar-thin">
            <input type="text" 
                   className="w-full bg-light-100 border-b border-light-400 outline-none pl-4"
                   placeholder="Search..."
                   value={query}
                   onChange={handleQueryChange}/>
            {/* <div onClick={selectAll} className="flex justify-between px-4 items-center hover:bg-light-100 cursor-pointer">
                <p>All</p>
            </div> */}
